# Claude Code Prompt — Apply New Orders Page

Paste this into Claude Code (in the `~/la-sirene-app` directory):

---

Replace the entire contents of `src/app/(app)/orders/page.tsx` with the following code. This is a complete rewrite that adds two separate sections: "My Appointments" (all statuses, with draft/cancel actions) and "My Orders" (business-created orders).
